#!/usr/bin/env bash
# Train and evaluate the standard Airfoil benchmark with Transolver.
set -euo pipefail
...
for DATA_FILE in NACA_Cylinder_X.npy NACA_Cylinder_Y.npy NACA_Cylinder_Q.npy; do
  if [[ ! -s "$DATA_ROOT/airfoil/naca/$DATA_FILE" ]]; then
    echo "ERROR: missing standard Airfoil data: $DATA_ROOT/airfoil/naca/$DATA_FILE" >&2
    exit 1
  fi
done
RUN_STAMP="$(date '+%Y%m%d_%H%M%S')"
EXPERIMENT_DIR="$OUTPUT_ROOT/${RUN_STAMP}_airfoil_Transolver"
SUFFIX=0
while [[ -e "$EXPERIMENT_DIR" ]]; do
  SUFFIX=$((SUFFIX + 1))
  EXPERIMENT_DIR="$OUTPUT_ROOT/${RUN_STAMP}_airfoil_Transolver_${SUFFIX}"
done
...
echo "Standard Airfoil training and evaluation completed: $EXPERIMENT_DIR"
