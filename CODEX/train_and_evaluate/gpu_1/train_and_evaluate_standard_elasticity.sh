#!/usr/bin/env bash
# Train and evaluate the standard Elasticity benchmark with Transolver.
set -euo pipefail
...
for DATA_FILE in elasticity/Meshes/Random_UnitCell_sigma_10.npy elasticity/Meshes/Random_UnitCell_XY_10.npy; do
  if [[ ! -s "$DATA_ROOT/$DATA_FILE" ]]; then
    echo "ERROR: missing standard Elasticity data: $DATA_ROOT/$DATA_FILE" >&2
    exit 1
  fi
done
RUN_STAMP="$(date '+%Y%m%d_%H%M%S')"
EXPERIMENT_DIR="$OUTPUT_ROOT/${RUN_STAMP}_elas_Transolver"
SUFFIX=0
while [[ -e "$EXPERIMENT_DIR" ]]; do
  SUFFIX=$((SUFFIX + 1))
  EXPERIMENT_DIR="$OUTPUT_ROOT/${RUN_STAMP}_elas_Transolver_${SUFFIX}"
done
...
python exp_elas.py \
  ...
  --lr 0.001 --max_grad_norm 0.1 --batch-size 1 \
  --slice_num 64 --unified_pos 0 --ref 8 --downsample 5 \
...
echo "Standard Elasticity training and evaluation completed: $EXPERIMENT_DIR"
