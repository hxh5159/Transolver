...
### Checkpoints and training visualizations

Training saves a state-dict checkpoint every 50 epochs by default. Checkpoints include the model, optimizer,
learning-rate scheduler, normalization coefficients, and complete loss history. Outputs are grouped by model,
fold, epoch count, and pressure-loss weight under `./ckpt`, for example:

```text
ckpt/Transolver/fold_0/200_0.5/
...
The field plots contain only supervised MLCFD outputs: velocity on all nodes and pressure on car-surface
nodes. Streamlines are derived from the predicted velocity components, and `validation_cd.png` uses the
existing pressure-and-surface-velocity drag postprocessing. Use `--checkpoint_interval` and
`--visualization_interval` to change the default interval. Resume a run with
`--resume /path/to/checkpoint_epoch_0050.pth` while keeping the original training hyperparameters.

Evaluation keeps the original metrics and prediction arrays, and also writes aggregate plots and
per-sample metrics under the evaluated fold and epoch:

```text
results/Transolver/evaluation/fold_0/epoch_0200/
├── l2_error_distribution.png
...
Pressure metrics and plots use car-surface nodes, while volume-velocity metrics use non-surface nodes.
The three representative cases are selected by relative `Cd` error and contain surface pressure,
volume velocity, and velocity-streamline ground truth, prediction, and error views. Inference latency
measures only the model forward call at batch size 1; it excludes data loading, GPU transfer,
denormalization, file output, and `Cd` postprocessing.
