...
'''
计算网格点的法向量,但法向量只在表面网格计算，
而且这里计算的是表面片的法向量而不是点的法向量，点的法向量是通过片的法向量插值得到的
在获取法向量时：vtkPolyDataNormals。作用：同向，连续，计算表面片法向量
'''
...
'''
加载两个vtk文件，分别是压力场和速度场，提取网格点坐标、压力、速度、法向量、sdf、边索引等信息，并保存为npy文件
与翼型不同，需分别对表面和体积各保存一个
是否添加preprocessded的目的是为了避免每次都重新计算这些信息，节省时间（True会直接在混村中加载)
'''
