#!/usr/bin/env bash
# =============================================================================
# Plasticity（标准基准）单卡训练 + 评估，使用论文配置。
#
# 论文配置（scripts/Transolver_Plas.sh）：model=Transolver_Structured_Mesh_2D,
...
DATA_PATH="$DATA/fno/plas_N987_T20.mat"   # exp_plas.py 的 --data_path 直接指向 .mat 文件

CONDA_ENV="${CONDA_ENV:-transolver}"
GPU="${GPU:-0}"
SEED="${SEED:-0}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-plas_Transolver}"
...
if [ ! -f "$BENCH_DIR/exp_plas.py" ]; then echo "错误：找不到 $BENCH_DIR/exp_plas.py"; exit 1; fi
if [ ! -s "$DATA_PATH" ]; then echo "错误：找不到 Plasticity 数据 $DATA_PATH"; exit 1; fi
cd "$BENCH_DIR"
...
python exp_plas.py "${COMMON_ARGS[@]}" --eval 0 || { log "训练失败"; exit 1; }

log "开始评估"
python exp_plas.py "${COMMON_ARGS[@]}" --eval 1 || { log "评估失败"; exit 1; }
