#!/usr/bin/env bash
# =============================================================================
# Airfoil（标准基准）深度消融：依次训练 block=4,5,6,7 的 Transolver（单卡 GPU 1）
# 论文配置除 --n-layers 外固定；输出到 depth_ablation/output/airfoil/block_<N>/
# =============================================================================
set -uo pipefail
...
[ -f "$BENCH_DIR/exp_airfoil.py" ] || { echo "错误：找不到 $BENCH_DIR/exp_airfoil.py"; exit 1; }
cd "$BENCH_DIR"
