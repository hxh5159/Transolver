#!/usr/bin/env bash
# =============================================================================
# Pipe（标准基准）单卡训练 + 评估，使用论文配置。
#
# 论文配置（scripts/Transolver_Pipe.sh）：model=Transolver_Structured_Mesh_2D,
#   n_hidden=128, n_heads=8, n_layers=8, mlp_ratio=2, lr=0.001, max_grad_norm=0.1,
#   batch_size=8, slice_num=64, unified_pos=0, ref=8
...
DATA_PATH="$DATA/fno/pipe"   # 内含 Pipe_{X,Y,Q}.npy

CONDA_ENV="${CONDA_ENV:-transolver}"
GPU="${GPU:-0}"
SEED="${SEED:-0}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-pipe_Transolver}"
...
if [ ! -f "$BENCH_DIR/exp_pipe.py" ]; then echo "错误：找不到 $BENCH_DIR/exp_pipe.py"; exit 1; fi
if [ ! -s "$DATA_PATH/Pipe_X.npy" ]; then
  echo "错误：找不到 Pipe 数据 $DATA_PATH/Pipe_*.npy"
  exit 1
fi
cd "$BENCH_DIR"
...
COMMON_ARGS=(
  ...
  --mlp_ratio "$MLP_RATIO" --lr "$LR" --max_grad_norm "$MAX_GRAD_NORM" --batch-size "$BATCH_SIZE"
...
python exp_pipe.py "${COMMON_ARGS[@]}" --eval 0 || { log "训练失败"; exit 1; }

log "开始评估"
python exp_pipe.py "${COMMON_ARGS[@]}" --eval 1 || { log "评估失败"; exit 1; }
