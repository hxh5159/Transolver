...
artifact_paths = prepare_experiment(
    args.output_root, 'airfoil_' + save_name,
    args.experiment_dir or infer_experiment_dir(args.resume))
if eval:
    started_at = initialize_evaluation(artifact_paths, args, {'benchmark': 'airfoil'})
else:
    run_config, started_at = initialize(artifact_paths, args, {'benchmark': 'airfoil'})
...
def main():
    ...
    if eval:
        ...
        visualization_metrics = finalize_scalar_evaluation(
            ...
            grid_shape=(s1, s2), field_name='Airflow velocity', show_mesh=True)
        write_json(os.path.join(artifact_paths['evaluation'], 'evaluation_metrics.json'), {
            'status': 'completed', 'benchmark': 'airfoil',
    ...
    # 训练流程
        ...
        for ep in range(start_epoch, args.epochs):
            ...
            record_epoch(artifact_paths, history, ep + 1, epoch_started,
                         train_loss=float(train_loss), test_relative_l2=float(rel_err))
            ...
            if args.checkpoint_interval > 0 and (ep + 1) % args.checkpoint_interval == 0:
                ...
                save_checkpoint(artifact_paths, ep + 1, model, optimizer, scheduler,
                                history, metadata=vars(args))

        print('save model')
        finalize_training(artifact_paths, args.epochs, model, optimizer, scheduler,
                          history, vars(args), started_at, args.epochs, ntrain, ntest,
                          extra={'final_train_loss': float(train_loss),
